describe('L.mapbox.geocoderControl', function() {
    ...
    it('performs forward geolocation, centering the map on the first result', function() {
        var map = new L.Map(document.createElement('div')),
            control = L.mapbox.geocoderControl('https://api.tiles.mapbox.com/v3/examples.map-i875kd35/geocode/{query}.json').addTo(map);

        expect(control instanceof L.mapbox.GeocoderControl).to.eql(true);

        server.respondWith('GET',
            'https://api.tiles.mapbox.com/v3/examples.map-i875kd35/geocode/austin.json',
    ...
    it('performs forward geolocation, centering the map on the first result even if a point', function() {
        ...
        server.respondWith('GET',
            'https://api.tiles.mapbox.com/v3/examples.map-i875kd35/geocode/white%20house.json',
    ...
    it('supports the pointzoom option for preferred zoom for point results', function() {
        var map = new L.Map(document.createElement('div')),
            control = L.mapbox.geocoderControl('https://api.tiles.mapbox.com/v3/examples.map-i875kd35/geocode/{query}.json', {
                pointZoom: 10
            }).addTo(map);
    ...
    it('sets url based on an id', function() {
        ...
        expect(control.getURL()).to.equal('https://a.tiles.mapbox.com/v3/examples.map-i875kd35/geocode/{query}.json');
    });
    ...
    it('#setID', function() {
        ...
        expect(control.getURL()).to.equal('https://a.tiles.mapbox.com/v3/foobar/geocode/{query}.json');
    });
    ...
    describe('events', function() {
        ...
        beforeEach(function() {
            ...
            control = L.mapbox.geocoderControl('https://example.com/{query}.json').addTo(map);
        });
